...
export default function Page(){
  ...
  const addToCart = (buyNow=false)=>{
    if(!selected) return;
    const item = {...selected, size, qty};
    setCart([...cart, item]);
    if(buyNow){ setView("cart"); pushView("cart"); }
    else { alert(`Added ${selected.name} Size:${size} Qty:${qty}`); }
  };
  ...
  const Footer = () => (
    <footer className="bg-black text-white mt-5 pt-5 pb-4 px-4">
      <div className="container-fluid"><div className="row g-4">
        <div className="col-12 col-md-5"><h6 className="fw-bold" style={{fontFamily:"serif"}}>IRENE'S CLOSET</h6><small className="text-secondary">Premium dresses, shirts, trousers & athletic wear.<br/>Wear Your Confidence • Countrywide delivery</small></div>
        <div className="col-6 col-md-3"><small className="fw-bold d-block mb-2">Shop</small><div className="small text-secondary d-flex flex-column gap-1"><span onClick={()=>change("All","shop")} style={{cursor:"pointer"}}>Shop</span><span onClick={()=>change("Men","shop")} style={{cursor:"pointer"}}>Men</span><span onClick={()=>change("Women","shop")} style={{cursor:"pointer"}}>Women</span><span onClick={()=>change("Unisex","shop")} style={{cursor:"pointer"}}>Unisex</span><span onClick={()=>change("Sports","sports")} style={{cursor:"pointer"}}>Sports</span></div></div>
        <div className="col-6 col-md-4"><small className="fw-bold d-block mb-2">Socials</small><div className="small text-secondary d-flex flex-column gap-1"><span>Instagram</span><span>TikTok</span><span>WhatsApp</span><span>Facebook</span></div></div>
      </div><div className="border-top border-secondary mt-4 pt-3 text-center"><small className="text-secondary">© 2026 Irene's Closet</small></div></div>
    </footer>
  );
          ...
          <div className="container-fluid p-0">
            <div className="row g-0 align-items-stretch" style={{minHeight:"55vh"}}>
              ...
              <div className="col-12 col-lg-7" style={{background:"#F5F1EB", minHeight:"400px", height:"60vh"}}>
                <img src="https://images.unsplash.com/photo-1485968579580-b6d095142e6e?w=800&q=80&auto=format&fit=crop" alt="" className="w-100 h-100" style={{objectFit:"cover", objectPosition:"top center", height:"100%", minHeight:"400px", display:"block"}}/>
              </div>
            </div>
          </div>
            ...
            <div className="row g-2 mt-3">
              {filtered.slice(0,8).map(p=>(
                <div key={p.id} className="col-6 col-md-3">
                  <div className="border" style={{cursor:"pointer"}} onClick={()=>openProduct(p)}>
                    <img src={p.img} className="w-100" style={{aspectRatio:"4/5",objectFit:"cover", objectPosition:"top center"}} alt=""/>
      ...
      {(view==="shop"||view==="sports"||view==="trending") && (
        <>
          ...
          <div className="relative w-full h-[420px] overflow-hidden bg-black">
            <img src="https://images.unsplash.com/photo-1489987707025-afc232f7ea0f?w=1600&q=80&auto=format&fit=crop" alt="" className="absolute inset-0 w-full h-full object-cover"/>
            <div className="absolute inset-0 bg-black/30"></div>
            <div className="relative z-10 h-full flex flex-col items-center justify-center text-white text-center px-3">
              <h1 className="text-[42px] font-bold" style={{fontFamily:"serif"}}>SHOP COLLECTION</h1>
              <p className="text-[13px] mt-2">Handpicked styles to elevate your everyday — confidence stitched in every piece.</p>
            </div>
          </div>
          <div className="container-fluid px-3 py-4">
            <div className="row g-2">
              {filtered.map(p=>(
                <div key={p.id} className="col-6 col-md-3">
                  <div className="border h-100" style={{cursor:"pointer"}} onClick={()=>openProduct(p)}>
                    <div className="position-relative"><img src={p.img} className="w-100" style={{aspectRatio:"4/5",objectFit:"cover", objectPosition:"top center"}} alt=""/><span className="position-absolute top-0 start-0 m-1 badge bg-warning text-dark" style={{fontSize:9}}>SAVE 22%</span></div>
                    <div className="p-2"><small className="fw-bold d-block" style={{fontSize:11,height:32, overflow:"hidden"}}>{p.name}</small><small>Shs {p.price.toLocaleString()}</small></div>
                  </div>
                </div>
              ))}
            </div>
        ...
        <>
          ...
          <div className="container-fluid px-3 py-3">
            <div className="row g-4">
              <div className="col-12 col-md-6"><img src={selected.img} className="w-100 rounded" style={{aspectRatio:"4/5",objectFit:"cover", objectPosition:"top center"}} alt=""/></div>
              <div className="col-12 col-md-6">
                <h3 className="fw-bold" style={{fontFamily:"serif", lineHeight:1.1}}>{selected.name}</h3>
                <h5 className="fw-bold mt-2">Shs {selected.price.toLocaleString()}</h5>
                <p className="text-secondary small mt-2">{selected.desc}</p>
                <small className="text-secondary">Haven</small>

                <div className="mt-3">
                  <div className="row g-2" style={{maxWidth:340}}>
                    {SIZES.map(s=>(
                      <div key={s} className="col-3 col-md-2">
                        <button onClick={()=>setSize(s)} style={{width:"100%", height:42, borderRadius:12, border: size===s? "2px solid #000":"1px solid #ccc", background: size===s? "#000":"#fff", color: size===s? "#fff":"#000", fontWeight:700}}>{s}</button>
                      </div>
                    ))}
                  </div>
                </div>

                <div className="mt-4 d-flex gap-2" style={{maxWidth:420}}>
                  <div className="d-flex align-items-center bg-light border" style={{height:44}}>
                    <button onClick={()=>setQty(q=>Math.max(1,q-1))} className="btn px-3">-</button>
                    <span className="px-2 fw-bold">{qty}</span>
                    <button onClick={()=>setQty(q=>q+1)} className="btn px-3">+</button>
                  </div>
                  <button onClick={()=>addToCart(false)} className="btn btn-dark flex-fill fw-bold d-flex align-items-center justify-content-center gap-2" style={{height:44}}>⎙ Add to cart</button>
                </div>

                <button onClick={()=>addToCart(true)} className="btn w-100 mt-2 fw-bold" style={{height:48, background:"#000", color:"#fff", maxWidth:420}}>Buy it now</button>
            ...
            {cart.length===0? <div className="text-center py-5"><p>Empty - continue touring shop</p><button onClick={()=>change("All","shop")} className="btn btn-dark rounded-pill">Shop Now</button></div> : (
              <div className="row g-3 mt-2">
                <div className="col-12 col-md-8">
                  {cart.map((c,i)=><div key={i} className="d-flex gap-3 border p-2 mb-2 align-items-center"><img src={c.img} style={{width:60,height:60,objectFit:"cover", objectPosition:"top center"}} alt=""/><div className="flex-fill"><small className="fw-bold">{c.name}</small><br/><small>Size:{c.size} Qty:{c.qty} - Shs {(c.price*c.qty).toLocaleString()}</small></div><button onClick={()=>setCart(cart.filter((_,idx)=>idx!==i))} className="btn btn-sm btn-outline-danger">X</button></div>)}
                </div>
                <div className="col-12 col-md-4">
                  <div className="border p-3 rounded">
                    <h6 className="fw-bold">Total: Shs {cart.reduce((a,b)=>a+(b.price*b.qty),0).toLocaleString()}</h6>
        ...
        <>
          ...
          <div className="container-fluid d-flex justify-content-center" style={{minHeight:"75vh"}}>
            <div style={{maxWidth:420,width:"100%",padding:"20px"}}>
              <div className="text-center"><div style={{fontSize:32}}>👗</div><h6 className="fw-bold">IRENE'S CLOSET</h6><h1 className="fw-bold mt-2" style={{fontFamily:"serif",fontSize:32}}>CREATE ACCOUNT</h1><small className="text-secondary">Required only for checkout</small></div>
              <button onClick={googleLogin} className="btn w-100 mt-4 d-flex justify-content-center align-items-center gap-2" style={{border:"1.5px solid #000",borderRadius:50,padding:12,fontWeight:700,fontSize:12}}><img src="https://www.svgrepo.com/show/475656/google-color.svg" style={{width:18}} alt=""/> CONTINUE WITH GOOGLE</button>
