...
// 24 IMAGES PER CATEGORY - CLOTHES ONLY!
const C = {
...
const make = (sec:string, n:number) => Array.from({length:n},(_,i)=>({
  ...
  img:`https://images.unsplash.com/${(C as any)[sec][i%(C as any)[sec].length]}?w=500`,
  save:`SAVE ${22+i%18}%`, section:sec
}));

const PRODUCTS = [...make("Men",24),...make("Women",24),...make("Unisex",24),...make("Sports",24)];
const HOME_BG = "/hero.jpg"; // YOUR CIRCLE CLOTHES IMAGE - same as Unisex!
...
export default function Page(){
  const [view,setView]=useState("home"); const [sec,setSec]=useState("All"); const [cart,setCart]=useState<any[]>([]); const [email,setEmail]=useState("");
  ...
  return(
    <div style={{background:"#fff"}}>
      <nav className="d-flex justify-content-between align-items-center px-3 py-3 bg-white sticky-top border-bottom" style={{zIndex:50}}>
        ...
        <div className="d-flex gap-2 d-md-none"><span onClick={()=>change("All","shop")} style={{cursor:"pointer}}>☰</span><span>🛒 {cart.length}</span></div><div className="d-none d-md-block">🛒 {cart.length}</div>
      </nav>

      {view==="home" && (
        <>
          {/* BEAUTIFUL HOME - CIRCLE CLOTHES LIKE PRIME WEAR */}
          <div style={{minHeight:"95vh",backgroundImage:headerBg,backgroundSize:"cover",backgroundPosition:"center"}} className="d-flex flex-column justify-content-center align-items-center text-center p-3">
            ...
            <div className="mt-5"><small className="text-white" style={{letterSpacing:4,fontSize:10}}>SHOP BY DEPARTMENT</small><div className="d-flex gap-2 mt-3 flex-wrap justify-content-center">{["MEN","WOMEN","UNISEX","SPORTS"].map(l=><button key={l} onClick={()=>change(l==="MEN"?"Men":l==="WOMEN"?"Women":l==="SPORTS"?"Sports":"Unisex",l==="SPORTS"?"sports":"shop")} className="btn rounded-pill px-4 py-2 fw-bold" style={{background:"rgba(0,0,0,0.35)",color:"white",border:"1px solid rgba(255,255,255,0.6)",backdropFilter:"blur(8px)",fontSize:11}}>{l}</button>)}</div></div>
          </div>

          <div className="container-fluid px-3 py-5 text-center" style={{background:"#FFFBF7"}}><small style={{letterSpacing:4,color:"#FF6A00"}}>WHY IRENE'S?</small><div className="row mt-4 g-3"><div className="col-4"><div className="fw-bold">🚚</div><small className="fw-bold">Countrywide</small><br/><small className="text-secondary" style={{fontSize:10}}>Delivered fast</small></div><div className="col-4"><div className="fw-bold">✨</div><small className="fw-bold">Premium Fit</small><br/><small className="text-secondary" style={{fontSize:10}}>Picked for quality</small></div><div className="col-4"><div className="fw-bold">💳</div><small className="fw-bold">Pay on Delivery</small><br/><small className="text-secondary" style={{fontSize:10}}>Cash / Mobile</small></div></div></div>

          <div className="container-fluid px-3 py-4"><h5 className="fw-bold">TRENDING NOW • 24 ITEMS</h5><div className="row g-2 mt-2">{filtered.slice(0,8).map(p=><div key={p.id} className="col-6 col-md-3"><div className="border"><img src={p.img} className="w-100" style={{aspectRatio:"1",objectFit:"cover"}} alt=""/><div className="p-2"><small className="fw-bold" style={{fontSize:11}}>{p.name}</small><br/><small className="fw-bold">Shs {p.price.toLocaleString()}</small><button onClick={()=>setCart([...cart,p])} className="btn btn-dark w-100 btn-sm rounded-0 mt-2">Add to Cart</button></div></div></div>)}</div><div className="text-center mt-3"><button onClick={()=>change("All","trending")} className="btn btn-outline-dark rounded-pill px-4">View All 24 Trending</button></div></div>
        </>
      )}
      ...
      {(view==="shop"||view==="sports"||view==="trending") && (
        <>
          <div className="text-center py-5 d-flex flex-column justify-content-center align-items-center px-3" style={{minHeight:380,backgroundImage:headerBg,backgroundSize:"cover",backgroundPosition:"center"}}>
            ...
            <p className="text-white mt-2">{isSports?"Built For Performance On Every Surface.":view==="trending"?"24 hottest clothes right now.":sec==="Unisex"?"Same background as Home — Genderless silhouettes.":"Premium clothes — 24 items per page."}</p>
          </div>
          <div className="container-fluid px-3 py-4"><div className="row"><div className="col-12 col-md-2 mb-3"><small className="fw-bold">Section</small><div className="mt-2 small d-flex flex-row flex-md-column gap-2 overflow-auto" style={{whiteSpace:"nowrap"}}><span onClick={()=>change("All","shop")} style={{cursor:"pointer"}}>All Products (24)</span><span onClick={()=>change("Men","shop")} style={{cursor:"pointer"}}>Men (24)</span><span onClick={()=>change("Women","shop")} style={{cursor:"pointer"}}>Women (24)</span><span onClick={()=>change("Unisex","shop")} style={{cursor:"pointer"}}>Unisex (24)</span><span onClick={()=>change("Sports","sports")} style={{cursor:"pointer"}}>Sports (24)</span><span onClick={()=>change("All","trending")} style={{cursor:"pointer",color:"#FF6A00"}}>Trending (24)</span></div></div><div className="col-12 col-md-10"><small className="mb-3 d-block">{filtered.length} Products • All Clothes Only</small><div className="row g-2">{filtered.map(p=><div key={p.id} className="col-6 col-md-4 col-lg-3"><div className="border h-100 bg-white"><span className="bg-warning small fw-bold px-2 py-1 m-1 rounded-2 position-absolute" style={{fontSize:9,zIndex:2}}>{p.save}</span><img src={p.img} className="w-100" style={{aspectRatio:"1",objectFit:"cover"}} alt=""/><div className="p-2"><small className="fw-bold d-block" style={{height:32,fontSize:11}}>{p.name}</small><small className="fw-bold">Shs {p.price.toLocaleString()}</small><button onClick={()=>setCart([...cart,p])} className="btn btn-dark w-100 btn-sm rounded-0 mt-2">Add to Cart</button></div></div></div>)}</div><div className="text-center mt-4"><button onClick={()=>change("All","home")} className="btn btn-outline-dark rounded-0">← Back Home</button></div></div></div></div>
        </>
      )}
    </div>
  );
